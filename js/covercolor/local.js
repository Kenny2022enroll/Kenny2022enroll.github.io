        ...
        if (r && g && b) {
            ...
            if (brightness < 125) {
                ...
                for (let i = 0; i < cardContents.length; i++) {
                    cardContents[i].style.setProperty('--flec-card-bg', 'var(--flec-white)');
                }

                let authorInfo = document.getElementsByClassName('author-info__sayhi');
                for (let i = 0; i < authorInfo.length; i++) {
                    authorInfo[i].style.setProperty('background', 'var(--flec-white-op)');
                    authorInfo[i].style.setProperty('color', 'var(--flec-white)');
                }
            }
        }
